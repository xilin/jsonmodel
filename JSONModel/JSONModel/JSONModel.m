...
-(BOOL)__doesDictionary:(NSDictionary*)dict matchModelWithKeyMapper:(JSONKeyMapper*)keyMapper error:(NSError**)err
{
    ...
    if (keyMapper || globalKeyMapper) {
        ...
        for (JSONModelClassProperty* property in [self __properties__]) {
            ...
            transformedName = (keyMapper||globalKeyMapper) ? [self __mapString:property.name withKeyMapper:keyMapper forDictionary:dict] : property.name;
...
-(NSString*)__mapString:(NSString*)string withKeyMapper:(JSONKeyMapper*)keyMapper forDictionary:(NSDictionary *)jsonDictionary
{
    if (keyMapper) {
        //custom mapper
        NSString* mappedName = [keyMapper convertValue:string forDictionary:jsonDictionary];
        if (globalKeyMapper && [mappedName isEqualToString: string]) {
            mappedName = [globalKeyMapper convertValue:string forDictionary:jsonDictionary];
        }
        string = mappedName;
    } else if (globalKeyMapper) {
        //global keymapper
        string = [globalKeyMapper convertValue:string forDictionary:jsonDictionary];
    }
...
-(BOOL)__importDictionary:(NSDictionary*)dict withKeyMapper:(JSONKeyMapper*)keyMapper validation:(BOOL)validation error:(NSError**)err
{
    ...
    for (JSONModelClassProperty* property in [self __properties__]) {
        ...
        NSString* jsonKeyPath = (keyMapper||globalKeyMapper) ? [self __mapString:property.name withKeyMapper:keyMapper forDictionary:dict] : property.name;
...
- (NSDictionary *)toDictionaryWithKeys:(NSArray <NSString *> *)propertyNames
{
    ...
    for (JSONModelClassProperty* p in properties) {
        ...
        //fetch key and value
        NSString* keyPath = (self.__keyMapper||globalKeyMapper) ? [self __mapString:p.name withKeyMapper:self.__keyMapper forDictionary:nil] : p.name;
